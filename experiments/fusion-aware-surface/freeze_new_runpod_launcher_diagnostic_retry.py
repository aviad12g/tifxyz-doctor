#!/usr/bin/env python3
"""Freeze the result-blind launcher-exception diagnostic CPU retry."""
...
PREDECESSOR_PAYLOAD = "2bae30c429b11ee627a15d45895ad8f6109d5571a13616a0afa73046c328df11"
FAILED_RECEIPT_PAYLOAD = "d31e57bed67fce89d7edfa12db4726c486f469b2547d1ea2c35c47903aca9d98"
...
def write_hashed(path: Path, payload: dict) -> None:
    if path.exists():
        raise RuntimeError("launcher-diagnostic retry plan target must start absent")
    content = dict(payload)
    content.pop("payload_sha256", None)
...
def main() -> int:
    ...
    if predecessor["payload_sha256"] != PREDECESSOR_PAYLOAD:
        raise RuntimeError("wrong corrected-executor predecessor plan")
    if (
        receipt["payload_sha256"] != FAILED_RECEIPT_PAYLOAD
        or receipt.get("status") != "RunPod CPU real-scoring allocation terminated after sealed copy"
        or receipt.get("scientific_outputs_inspected") is not False
    ):
        raise RuntimeError("wrong result-blind launcher failure receipt")
    plan = deepcopy(predecessor)
    plan["status"] = "fresh CPU launcher-diagnostic retry frozen before create"
    ...
    plan["budget"].update(
        {
            "prior_launcher_failure_guarded_allowance_usd": 0.5,
            "compute_cutoff_usd": 15.0,
            "reserve_usd": 1.0,
            "maximum_provider_wall_seconds_at_price_ceiling": 42_187,
            "maximum_runtime_seconds_before_guard_at_price_ceiling": 41_887,
        }
    )
    plan["result_blind_launcher_diagnostic_retry"] = {
        "schema_version": "1.0",
        "status": "bounded operational launcher diagnostic frozen before CPU create",
        "failed_receipt": {
            ...
            "launcher_returned_nonzero": True,
            "scientific_outputs_inspected": False,
        },
        "correction": {
            "project_only_exception_type_and_message_lines": True,
            "maximum_projected_line_bytes": 1_024,
            "maximum_projected_lines": 3,
            "scientific_stdout_projected": False,
            "all_existing_identity_and_runtime_checks_retained": True,
    ...
    print("NEW_RUNPOD_LAUNCHER_DIAGNOSTIC_RETRY_FROZEN")
