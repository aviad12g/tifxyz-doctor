#!/usr/bin/env python3
"""Freeze the CPU retry with the exact public metric-worker sibling."""
...
PREDECESSOR_PAYLOAD = "3b22c5216cf8ed6b9cd03ca18563263f470e2ee6dcc99c32ffcee3bc8c07b393"
FAILED_RECEIPT_PAYLOAD = "9f1a1a4371e4145b0bb5e09560aa880ac153b18c27ce63219d5e7513afa6b028"
...
def write_hashed(path: Path, payload: dict) -> None:
    if path.exists():
        raise RuntimeError("metric-worker retry plan target must start absent")
    content = dict(payload)
    content.pop("payload_sha256", None)
...
def main() -> int:
    ...
    if (
        ...
        or receipt.get("pod_id") != "g6napb4zi036b1"
    ):
        raise RuntimeError("wrong result-blind missing-metric-worker receipt")
    plan = deepcopy(predecessor)
    plan["status"] = "fresh CPU exact public metric-worker retry frozen before create"
    ...
    plan["budget"].update(
        {
            "prior_missing_metric_worker_guarded_allowance_usd": 0.8,
            "compute_cutoff_usd": 8.2,
            "reserve_usd": 1.0,
            "maximum_provider_wall_seconds_at_price_ceiling": 23_062,
            "maximum_runtime_seconds_before_guard_at_price_ceiling": 22_762,
        }
    )
    plan["result_blind_public_metric_worker_retry"] = {
        "schema_version": "1.0",
        "status": "exact hash-bound public official-metric sibling frozen before CPU create",
        "failed_receipt": {
        ...
        "scientific_contract": {
            "only_exact_existing_public_metric_worker_materialized": True,
    ...
    print("NEW_RUNPOD_PUBLIC_METRIC_WORKER_RETRY_FROZEN")
