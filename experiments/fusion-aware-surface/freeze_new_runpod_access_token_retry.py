#!/usr/bin/env python3
"""Freeze the current Kaggle access-token private transport correction."""
...
PREDECESSOR_PAYLOAD = "ef96d4256053e31521533f5a2a2b5b59bc02a73af861dbc6a66bdb04702d2615"
FAILED_RECEIPT_PAYLOAD = "8f28d835440edbc04b52f6047d210c4890457b05fee11b4a2c8675c747e030c1"
...
def write_hashed(path: Path, payload: dict) -> None:
    if path.exists():
        raise RuntimeError("access-token retry plan target must start absent")
    content = dict(payload)
    content.pop("payload_sha256", None)
...
def main() -> int:
    ...
    if predecessor["payload_sha256"] != PREDECESSOR_PAYLOAD:
        raise RuntimeError("wrong completion-marker-corrected predecessor plan")
    if (
    ...
    plan["status"] = "fresh CPU current-Kaggle-access-token retry frozen before create"
    ...
    plan["budget"].update(
        {
            "prior_private_auth_retry_operational_allowance_usd": 0.5,
            "compute_cutoff_usd": 18.5,
            "reserve_usd": 1.0,
            "maximum_provider_wall_seconds_at_price_ceiling": 52_031,
            "maximum_runtime_seconds_before_guard_at_price_ceiling": 51_731,
        }
    )
    plan["result_blind_current_kaggle_access_token_retry"] = {
        "schema_version": "1.0",
        "status": "current access-token private transport correction frozen before CPU create",
        "failed_receipt": {
            "file": args.failed_receipt.name,
            "payload_sha256": receipt["payload_sha256"],
            "pod_id": receipt["pod_id"],
            "failure_state": "PULLING_PRIVATE_TRANSPORT",
            "failure_type": "KaggleApiHTTPError",
        ...
        "correction": {
            "credential_kind": "Kaggle access token",
            "credential_value_or_hash_frozen_logged_or_recorded": False,
            "remote_filename": "access_token",
            "remote_mode": "0600",
            "authentication_environment": "KAGGLE_API_TOKEN points to ephemeral file",
    ...
    print("NEW_RUNPOD_CURRENT_KAGGLE_ACCESS_TOKEN_RETRY_FROZEN")
