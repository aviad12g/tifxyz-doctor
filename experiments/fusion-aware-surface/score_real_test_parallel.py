#!/usr/bin/env python3
"""Score sealed Scroll-4/5 caches concurrently in frozen cache order."""
...
def score_cache_set(
    ...
    """Run independent metric subprocesses and restore frozen cache order."""
    if not 1 <= parallel_workers <= MAX_PARALLEL_WORKERS:
        raise ValueError(f"parallel workers must be in [1, {MAX_PARALLEL_WORKERS}]")
