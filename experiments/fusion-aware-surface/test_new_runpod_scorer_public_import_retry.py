...
def test_public_import_retry_is_cpu_only_and_result_blind() -> None:
    source = (
        HERE / "freeze_new_runpod_scorer_public_import_retry.py"
...
def test_public_import_retry_names_the_exact_dependencies() -> None:
    source = (
        HERE / "freeze_new_runpod_scorer_public_import_retry.py"
    ).read_text(encoding="utf-8")
    for name in (
